...
public class LittleSearchEngine {
	...
	/*public static void main(String args[]) {
		...
		System.out.print("Enter in the name of the doc file: ");
		String doc = "docs.txt";
		System.out.print("Enter in the name of the noise words file: ");
		String noise = "noisewords.txt";
		
		LittleSearchEngine lse = new LittleSearchEngine();
		try {
			lse.makeIndex(doc, noise);
	...
	public String getKeyWord(String word) {
		...
		if (noiseWords.containsKey(result)) {
			return null;
		} else {
			return result;
		}
	...
	public ArrayList<Integer> insertLastOccurrence(ArrayList<Occurrence> occs) {
		...
		while(l != r) {
			m = (l+r)/2;
			result.add(m);
			if (occs.get(m).frequency > val) {
				l = m+1;
			} else if (occs.get(m).frequency < val) {
				r = m;
			} else if (occs.get(m).frequency == val) {
		...
		if (occs.get(l).frequency > occs.get(occs.size()-1).frequency) {
			return result;
		} else {
			occs.add(l, occs.remove(occs.size()-1));
			return result;
		}
	}
	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		ArrayList<Integer> size = new ArrayList<Integer>(5);
		for(Occurrence doc : keywordsIndex.get(kw1)) {
			for (int i = 0; i < 5; i++) {
				if (docs.get(i) == null || size.get(i) < doc.frequency) {
					docs.add(i, doc.document);
					docs.remove(docs.size()-1);
					size.add(i, doc.frequency);
					size.remove(size.size()-1);
					break;
				}
			}
		}
		for(Occurrence doc : keywordsIndex.get(kw2)) {
			for (int i = 0; i < 5; i++) {
				if (docs.get(i) == null || size.get(i) < doc.frequency) {
					docs.add(i, doc.document);
					docs.remove(docs.size()-1);
